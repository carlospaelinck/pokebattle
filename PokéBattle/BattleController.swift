...
class BattleController {
    func performAttack(
        ...
        let STAB = atkPkm.basePokémon.types.contains(atk.type) ? 1.5 : 1.0
        let attackType: PokémonType = atk.type
        let roll = (Double(arc4random_uniform(16) + 85) / 100)

        let damageMultiplierA = PokémonType.damageMultiplier(forAttackingType: attackType, andDefendingType: defPkm.basePokémon.types.first!)
        var damageMultiplierB: Double = 1

        if let secondaryType = defPkm.basePokémon.types.last where defPkm.basePokémon.types.count == 2 {
